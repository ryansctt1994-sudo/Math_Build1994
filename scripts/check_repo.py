...
that required files exist, that JSON policy/receipt-governance artifacts parse,
and that forbidden proof shortcuts are not present in local Lean source.
...
def main() -> int:
    ...
    for item in LEAN_GUARD_FILES:
        ...
        for token in FORBIDDEN_LEAN_TOKENS:
            if token in text:
                errors.append(f"forbidden Lean token in {item}: {token}")
